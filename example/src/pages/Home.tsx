...
import { pickGalleryPhoto, thumbSrc, measureImage } from '../pickImage';
...
export default function Home() {
  ...
  const attributeGallery = async () => {
    try {
      ...
      const srcW = size.w > 0 ? size.w : Math.max(faces[0].x2 + 1, 1);
      const srcH = size.h > 0 ? size.h : Math.max(faces[0].y2 + 1, 1);
      ...
      setAttributeResult({
        ...
        box: faces[0],
        cropB64,
        cropLandmarks: mapLandmarksToCrop(faces[0], srcW, srcH, 200, 200),
      });
